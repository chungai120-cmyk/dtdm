...
public class Server4 {
    ...
    public static class ConnectionHandler implements Runnable {
        ...
        private final String serverName = "Server4";
        ...
        public void run() {
            try {
                ...
                Server4.log("LỖI ConnectionHandler: " + e.getMessage() + "\n");
            } finally {
        ...
        public void runServer() {
            try {
                ...
                if (inLine == null || inLine.trim().isEmpty()) {
                    Server4.log("Nhận được gói tin rỗng, bỏ qua.\n");
                    return;
                }
                ...
                try {
                    ...
                    Server4.log("LỖI parse gói tin: " + e.getMessage() + " | Raw: " + inLine + "\n");
                    return;
                }
                ...
                Server4.log("Thông điệp nhận được:\n"
                ...
                int start = 0;
                int act   = 1;
                try { start = Integer.parseInt(st); }   catch (Exception e) { Server4.log("LỖI parse start: " + st + "\n"); }
                try { act   = Integer.parseInt(action); } catch (Exception e) { Server4.log("LỖI parse action: " + action + "\n"); }
                ...
                if (start == 0) {
                    ...
                    outStream.flush();
                    Server4.log("Reply client: " + replyMessage + "\n");

                    if (replyMessage.startsWith("Lỗi")) {
                        Server4.log("Dừng jeton do lỗi validate.\n\n");
                        return;
                    }
                    ...
                    Server4.log("Khởi động vòng tròn Jeton: gửi Locked → " + rount.table[vt].name + "\n\n");
                ...
                if (type.equals("Locked") && start != 4) {
                    Server4.log("Nhận Locked (start=" + start + "), chuyển tiếp đến " + rount.table[vt].name + "\n\n");
                    start++;
                    sendToNext(vt, "@$" + start + "|" + t + "|" + lamport + "|"
                ...
                if (type.equals("Locked") && start == 4) {
                    Server4.log("Locked hoàn tất vòng. Tạo bảng tạm. Gửi Temped đi ngược.\n\n");
                    int stt = 1; act++;
                ...
                if (type.equals("Temped") && start != 4) {
                    Server4.log("Nhận Temped (start=" + start + "), chuyển ngược.\n\n");
                    start++;
                    int tam = getPrevAlive(t, pos);
                ...
                if (type.equals("Temped") && start == 4) {
                    Server4.log("Temped hoàn tất. Cập nhật DB chính. Gửi Updated đi xuôi.\n\n");
                    int stt = 1; act++;
                    executeDatabaseAction(message);
                ...
                if (type.equals("Updated") && start != 4) {
                    Server4.log("Nhận Updated (start=" + start + "), chuyển tiếp đến " + rount.table[vt].name + "\n\n");
                    start++;
                    sendToNext(vt, "@$" + start + "|" + t + "|" + lamport + "|"
                ...
                if (type.equals("Updated") && start == 4) {
                    Server4.log("Updated hoàn tất. Gửi Synchronymed đi ngược.\n\n");
                    int stt = 1; act++;
                    int tam = getPrevAlive(t, pos);
                    sendToNext(tam, "@$" + stt + "|" + t + "|" + lamport + "|"
                ...
                if (type.equals("Synchronymed") && start != 4) {
                    Server4.log("Nhận Synchronymed (start=" + start + "), chuyển ngược.\n\n");
                    start++;
                    int tam = getPrevAlive(t, pos);
                    sendToNext(tam, "@$" + start + "|" + t + "|" + lamport + "|"
                ...
                if (type.equals("Synchronymed") && start == 4) {
                    Server4.log("✅ HOÀN TẤT GIAO DỊCH. Vòng tròn Jeton kết thúc.\n\n");
                }

            } catch (Exception e) {
                Server4.log("LỖI runServer: " + e.getClass().getSimpleName() + " - " + e.getMessage() + "\n");
            }
        }
        ...
        private int getPrevAlive(String jeton, int pos) {
            ...
            try {
                if (jeton.charAt(tam) == '0') {
                    Server4.log("Server" + (tam + 1) + " bị sự cố (jeton=" + jeton + "), thử server trước.\n");
                    tam--;
                    if (tam < 0) tam = rount.max - 1;
                }
            } catch (Exception e) {}
        ...
        private void sendToNext(int tableIndex, String msg) {
            ...
            for (int retry = 0; retry < rount.max; retry++) {
                try {
                    ...
                    Server4.log("⚠ " + rount.table[idx].name + " không phản hồi, thử server tiếp theo.\n");
                    idx = (idx + 1) % rount.max;
                }
            }
            Server4.log("❌ Tất cả server đều không phản hồi. Hủy jeton.\n");
        }
        ...
        private void executeDatabaseAction(String message) {
            try {
                ...
                if (message.endsWith("SET")) {
                    ...
                    Server4.log("DB: Đã thêm vé ghế " + data.getPos() + "\n");
                } else if (message.endsWith("DEL")) {
                    db.delData(data.getPos());
                    Server4.log("DB: Đã xóa vé ghế " + data.getPos() + "\n");
                }
            } catch (Exception e) {
                Server4.log("LỖI executeDatabaseAction: " + e.getMessage() + "\n");
            }
        }
    }
    ...
    public static class sv4 implements Runnable {
        ...
        sv4() {
            new Thread(this, "sv4-main").start();
        }
        ...
        public void run() {
            ...
            try {
                ...
                Server4.log("GetState khởi tạo thất bại (bỏ qua): " + e.getMessage() + "\n");
            }

            try (ServerSocket server = new ServerSocket(2004)) {
                Server4.log("=== Server 4 đang lắng nghe tại cổng 2004 ===\n");
                ...
                while (true) {
                    try {
                        ...
                        Server4.log("LỖI accept: " + e.getMessage() + "\n");
                    }
                }
            } catch (IOException e) {
                Server4.log("❌ Không thể mở cổng 2004: " + e.getMessage() + "\n");
            }
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        httpServer.createContext("/", exchange -> {
            String response = "<html><head><meta charset='UTF-8'>"
                    ...
                    + "<h2>MÁY CHỦ 4 - RẠP PHIM (Google Cloud)</h2>"
        ...
        Server4.log("Web Monitor: http://localhost:8080\n");
