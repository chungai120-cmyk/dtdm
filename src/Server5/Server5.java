...
public class Server5 {
    ...
    public static class ConnectionHandler implements Runnable {
        ...
        private final String serverName = "Server5";
        ...
        public void run() {
            try {
                ...
                Server5.log("LỖI ConnectionHandler: " + e.getMessage() + "\n");
            } finally {
        ...
        public void runServer() {
            try {
                ...
                if (inLine == null || inLine.trim().isEmpty()) {
                    Server5.log("Nhận được gói tin rỗng, bỏ qua.\n");
                    return;
                }
                ...
                try {
                    ...
                    Server5.log("LỖI parse gói tin: " + e.getMessage() + " | Raw: " + inLine + "\n");
                    return;
                }
                ...
                Server5.log("Thông điệp nhận được:\n"
                ...
                int start = 0;
                int act   = 1;
                try { start = Integer.parseInt(st); }   catch (Exception e) { Server5.log("LỖI parse start: " + st + "\n"); }
                try { act   = Integer.parseInt(action); } catch (Exception e) { Server5.log("LỖI parse action: " + action + "\n"); }
                ...
                if (start == 0) {
                    ...
                    outStream.flush();
                    Server5.log("Reply client: " + replyMessage + "\n");

                    if (replyMessage.startsWith("Lỗi")) {
                        Server5.log("Dừng jeton do lỗi validate.\n\n");
                        return;
                    }
                    ...
                    Server5.log("Khởi động vòng tròn Jeton: gửi Locked → " + rount.table[vt].name + "\n\n");
                ...
                if (type.equals("Locked") && start != 4) {
                    Server5.log("Nhận Locked (start=" + start + "), chuyển tiếp đến " + rount.table[vt].name + "\n\n");
                    start++;
                    sendToNext(vt, "@$" + start + "|" + t + "|" + lamport + "|"
                ...
                if (type.equals("Locked") && start == 4) {
                    Server5.log("Locked hoàn tất vòng. Tạo bảng tạm. Gửi Temped đi ngược.\n\n");
                    int stt = 1; act++;
                ...
                if (type.equals("Temped") && start != 4) {
                    Server5.log("Nhận Temped (start=" + start + "), chuyển ngược.\n\n");
                    start++;
                    int tam = getPrevAlive(t, pos);
                ...
                if (type.equals("Temped") && start == 4) {
                    Server5.log("Temped hoàn tất. Cập nhật DB chính. Gửi Updated đi xuôi.\n\n");
                    int stt = 1; act++;
                    executeDatabaseAction(message);
                ...
                if (type.equals("Updated") && start != 4) {
                    Server5.log("Nhận Updated (start=" + start + "), chuyển tiếp đến " + rount.table[vt].name + "\n\n");
                    start++;
                    sendToNext(vt, "@$" + start + "|" + t + "|" + lamport + "|"
                ...
                if (type.equals("Updated") && start == 4) {
                    Server5.log("Updated hoàn tất. Gửi Synchronymed đi ngược.\n\n");
                    int stt = 1; act++;
                    int tam = getPrevAlive(t, pos);
                    sendToNext(tam, "@$" + stt + "|" + t + "|" + lamport + "|"
                ...
                if (type.equals("Synchronymed") && start != 4) {
                    Server5.log("Nhận Synchronymed (start=" + start + "), chuyển ngược.\n\n");
                    start++;
                    int tam = getPrevAlive(t, pos);
                    sendToNext(tam, "@$" + start + "|" + t + "|" + lamport + "|"
                ...
                if (type.equals("Synchronymed") && start == 4) {
                    Server5.log("✅ HOÀN TẤT GIAO DỊCH. Vòng tròn Jeton kết thúc.\n\n");
                }

            } catch (Exception e) {
                Server5.log("LỖI runServer: " + e.getClass().getSimpleName() + " - " + e.getMessage() + "\n");
            }
        }
        ...
        private int getPrevAlive(String jeton, int pos) {
            ...
            try {
                if (jeton.charAt(tam) == '0') {
                    Server5.log("Server" + (tam + 1) + " bị sự cố (jeton=" + jeton + "), thử server trước.\n");
                    tam--;
                    if (tam < 0) tam = rount.max - 1;
                }
            } catch (Exception e) {}
        ...
        private void sendToNext(int tableIndex, String msg) {
            ...
            for (int retry = 0; retry < rount.max; retry++) {
                try {
                    ...
                    Server5.log("⚠ " + rount.table[idx].name + " không phản hồi, thử server tiếp theo.\n");
                    idx = (idx + 1) % rount.max;
                }
            }
            Server5.log("❌ Tất cả server đều không phản hồi. Hủy jeton.\n");
        }
        ...
        private void executeDatabaseAction(String message) {
            try {
                ...
                if (message.endsWith("SET")) {
                    ...
                    Server5.log("DB: Đã thêm vé ghế " + data.getPos() + "\n");
                } else if (message.endsWith("DEL")) {
                    db.delData(data.getPos());
                    Server5.log("DB: Đã xóa vé ghế " + data.getPos() + "\n");
                }
            } catch (Exception e) {
                Server5.log("LỖI executeDatabaseAction: " + e.getMessage() + "\n");
            }
        }
    }
    ...
    public static class sv5 implements Runnable {
        ...
        sv5() {
            new Thread(this, "sv5-main").start();
        }
        ...
        public void run() {
            ...
            try {
                ...
                Server5.log("GetState khởi tạo thất bại (bỏ qua): " + e.getMessage() + "\n");
            }

            try (ServerSocket server = new ServerSocket(2005)) {
                Server5.log("=== Server 5 đang lắng nghe tại cổng 2005 ===\n");
                ...
                while (true) {
                    try {
                        ...
                        Server5.log("LỖI accept: " + e.getMessage() + "\n");
                    }
                }
            } catch (IOException e) {
                Server5.log("❌ Không thể mở cổng 2005: " + e.getMessage() + "\n");
            }
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        httpServer.createContext("/", exchange -> {
            String response = "<html><head><meta charset='UTF-8'>"
                    ...
                    + "<h2>MÁY CHỦ 5 - RẠP PHIM (Google Cloud)</h2>"
        ...
        Server5.log("Web Monitor: http://localhost:8080\n");
