...
public class Server2 {
    ...
    public static class ConnectionHandler implements Runnable {
        ...
        private final String serverName = "Server2";
        ...
        public void run() {
            try {
                ...
                Server2.log("LỖI ConnectionHandler: " + e.getMessage() + "\n");
            } finally {
        ...
        public void runServer() {
            try {
                ...
                if (inLine == null || inLine.trim().isEmpty()) {
                    Server2.log("Nhận được gói tin rỗng, bỏ qua.\n");
                    return;
                }
                ...
                try {
                    ...
                    Server2.log("LỖI parse gói tin: " + e.getMessage() + " | Raw: " + inLine + "\n");
                    return;
                }
                ...
                Server2.log("Thông điệp nhận được:\n"
                ...
                int start = 0;
                int act   = 1;
                try { start = Integer.parseInt(st); }   catch (Exception e) { Server2.log("LỖI parse start: " + st + "\n"); }
                try { act   = Integer.parseInt(action); } catch (Exception e) { Server2.log("LỖI parse action: " + action + "\n"); }
                ...
                if (start == 0) {
                    ...
                    outStream.flush();
                    Server2.log("Reply client: " + replyMessage + "\n");

                    if (replyMessage.startsWith("Lỗi")) {
                        Server2.log("Dừng jeton do lỗi validate.\n\n");
                        return;
                    }
                    ...
                    Server2.log("Khởi động vòng tròn Jeton: gửi Locked → " + rount.table[vt].name + "\n\n");
                ...
                if (type.equals("Locked") && start != 4) {
                    Server2.log("Nhận Locked (start=" + start + "), chuyển tiếp đến " + rount.table[vt].name + "\n\n");
                    start++;
                    sendToNext(vt, "@$" + start + "|" + t + "|" + lamport + "|"
                ...
                if (type.equals("Locked") && start == 4) {
                    Server2.log("Locked hoàn tất vòng. Tạo bảng tạm. Gửi Temped đi ngược.\n\n");
                    int stt = 1; act++;
                ...
                if (type.equals("Temped") && start != 4) {
                    Server2.log("Nhận Temped (start=" + start + "), chuyển ngược.\n\n");
                    start++;
                    int tam = getPrevAlive(t, pos);
                ...
                if (type.equals("Temped") && start == 4) {
                    Server2.log("Temped hoàn tất. Cập nhật DB chính. Gửi Updated đi xuôi.\n\n");
                    int stt = 1; act++;
                    executeDatabaseAction(message);
                ...
                if (type.equals("Updated") && start != 4) {
                    Server2.log("Nhận Updated (start=" + start + "), chuyển tiếp đến " + rount.table[vt].name + "\n\n");
                    start++;
                    sendToNext(vt, "@$" + start + "|" + t + "|" + lamport + "|"
                ...
                if (type.equals("Updated") && start == 4) {
                    Server2.log("Updated hoàn tất. Gửi Synchronymed đi ngược.\n\n");
                    int stt = 1; act++;
                    int tam = getPrevAlive(t, pos);
                    sendToNext(tam, "@$" + stt + "|" + t + "|" + lamport + "|"
                ...
                if (type.equals("Synchronymed") && start != 4) {
                    Server2.log("Nhận Synchronymed (start=" + start + "), chuyển ngược.\n\n");
                    start++;
                    int tam = getPrevAlive(t, pos);
                    sendToNext(tam, "@$" + start + "|" + t + "|" + lamport + "|"
                ...
                if (type.equals("Synchronymed") && start == 4) {
                    Server2.log("✅ HOÀN TẤT GIAO DỊCH. Vòng tròn Jeton kết thúc.\n\n");
                }

            } catch (Exception e) {
                Server2.log("LỖI runServer: " + e.getClass().getSimpleName() + " - " + e.getMessage() + "\n");
            }
        }
        ...
        private int getPrevAlive(String jeton, int pos) {
            ...
            try {
                if (jeton.charAt(tam) == '0') {
                    Server2.log("Server" + (tam + 1) + " bị sự cố (jeton=" + jeton + "), thử server trước.\n");
                    tam--;
                    if (tam < 0) tam = rount.max - 1;
                }
            } catch (Exception e) {}
        ...
        private void sendToNext(int tableIndex, String msg) {
            ...
            for (int retry = 0; retry < rount.max; retry++) {
                try {
                    ...
                    Server2.log("⚠ " + rount.table[idx].name + " không phản hồi, thử server tiếp theo.\n");
                    idx = (idx + 1) % rount.max;
                }
            }
            Server2.log("❌ Tất cả server đều không phản hồi. Hủy jeton.\n");
        }
        ...
        private void executeDatabaseAction(String message) {
            try {
                ...
                if (message.endsWith("SET")) {
                    ...
                    Server2.log("DB: Đã thêm vé ghế " + data.getPos() + "\n");
                } else if (message.endsWith("DEL")) {
                    db.delData(data.getPos());
                    Server2.log("DB: Đã xóa vé ghế " + data.getPos() + "\n");
                }
            } catch (Exception e) {
                Server2.log("LỖI executeDatabaseAction: " + e.getMessage() + "\n");
            }
        }
    }
    ...
    public static class sv2 implements Runnable {
        ...
        sv2() {
            new Thread(this, "sv2-main").start();
        }
        ...
        public void run() {
            ...
            try {
                ...
                Server2.log("GetState khởi tạo thất bại (bỏ qua): " + e.getMessage() + "\n");
            }

            try (ServerSocket server = new ServerSocket(2002)) {
                Server2.log("=== Server 2 đang lắng nghe tại cổng 2002 ===\n");
                ...
                while (true) {
                    try {
                        ...
                        Server2.log("LỖI accept: " + e.getMessage() + "\n");
                    }
                }
            } catch (IOException e) {
                Server2.log("❌ Không thể mở cổng 2002: " + e.getMessage() + "\n");
            }
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        httpServer.createContext("/", exchange -> {
            String response = "<html><head><meta charset='UTF-8'>"
                    ...
                    + "<h2>MÁY CHỦ 2 - RẠP PHIM (Google Cloud)</h2>"
        ...
        Server2.log("Web Monitor: http://localhost:8080\n");
